...
* **Glass morphism** — frosted layers (`backdrop-filter: blur(22px) saturate(160%)`) on translucent surfaces, layered above ambient gradient orbs for depth.
* **Dark theme by default**; toggle button (top-right header & login) swaps to a soft light palette and persists in `localStorage`.
* **Header**: gradient logo mark, brand wordmark, central nav tabs.
* **Footer**: pinned to the viewport bottom, centered text `CREATED BY | CHINMOY C.`
* **Charts**: Chart.js — doughnut, bar, polar-area, gradient-filled line.
...
└── frontend/
    ...
    ├── css/styles.css              # glass-morphism + theme tokens
...
2. **Switch theme** with the sun/moon toggle. Glass, gradients, and charts
   restyle live.
...
6. **Calendar** tab → reschedule an appointment via the custom dark date/time
